...
extension AbstractActionSheetPicker {
    func xt_defaultConfig() {
        titleTextAttributes = [kCTFontAttributeName : UIFont.fixedAppBoldFont (size: 18)]
        tapDismissAction = .cancel
        let cancelButton = UIBarButtonItem(title: "Cancle", style: .plain, target: nil, action: nil)
        cancelButton.setTitleTextAttributes([NSAttributedString.Key.font : UIFont.fixedAppFont(size: 14), NSAttributedString.Key.foregroundColor : UIColor.MyTheme.purple], for: UIControl.State.normal)
        let doneButton = UIBarButtonItem(title: "Done", style: .plain, target: nil, action: nil)
        doneButton.setTitleTextAttributes([NSAttributedString.Key.font : UIFont.fixedAppBoldFont(size: 14), NSAttributedString.Key.foregroundColor : UIColor.MyTheme.purple], for: UIControl.State.normal)
